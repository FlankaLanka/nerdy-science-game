...
`App.tsx` coordinates walking, close-up, notebook and pause. There are no ordinary network requests beyond local runtime assets. `tandemScript.ts` contains complete authored scenes with individual speech beats. `tandemDialogue.ts` selects state-aware E conversations, graduated hints shared with T, and physics-dependent reactions. `tandemDirector.ts` queues whole scenes, handles listening pauses and interruptions, checkpoints completed replies, and lets repair payoffs continue across doors. `useTandem.ts` connects the director to one caption and local voice at a time; actual recording completion controls spoken pacing, with readable caption timing as fallback. Menus and hidden tabs pause both. Heard identifiers and audio URLs are versioned with the script; puzzle progress and older memories remain compatible. `scene/tandem.ts` supplies the physical companion; bounded pathfinding in `guideNavigation.ts` uses the station's current collision geometry, including moving doors. Doors sense Tandem and preserve clearance while it crosses.
