...
| Six effects in `public/audio/` | Generated with ElevenLabs Sound Effects v2 from project-authored prompts; trimmed and levelled locally | Generated under the account's [ElevenLabs terms](https://elevenlabs.io/terms-of-use); not a CC0 library | Doors, electrical controls and ventilation |
| `public/audio/station-music.mp3` | Generated with Eleven Music v2 from the project-authored prompt in `scripts/music-design.json` | Generated under the account's [ElevenLabs music terms](https://elevenlabs.io/music-terms) | Calm instrumental background music |
...
The seven runtime files total **1,762,222 bytes**. Five short mono MP3s cover opening and closing doors, power startup, a connector and a relay. `ventilation.wav` is a 5.4-second mono loop with overlapping ends; PCM preserves its loop boundary without encoder padding. UI, discovery, success and fault cues use short Web Audio envelopes. Footstep samples and their playback path have been removed.

`station-music.mp3` is a 76-second stereo instrumental loop, 1,217,558 bytes at 128 kbps. Its prompt calls for warm electric piano, rounded synth plucks, airy pads and consonant harmony suitable for quiet exploration and concentration. A four-second equal-power overlap joins the ends before export, with headroom and low-frequency filtering. The browser checks the decoded loop boundary. Music fades in after play begins, drops in menus, follows Sound mute and is disposed on a new run.
...
Music generation uses `scripts/generate-station-music.py` and the [ElevenLabs music API](https://elevenlabs.io/docs/api-reference/music/compose). Its prompt and instrumental setting are in `scripts/music-design.json`. `scripts/prepare-station-music.py` prepares the local loop using Python/numpy and ffmpeg. Source audio and generation metadata remain in ignored `artifacts/audio-source/`.
