...
HUD content is limited to a short objective, compass, reticle, next-repair marker, interaction prompt, and a small power count. Story communication is brief radio text. The UI uses old ivory rag paper with transparent torn fibers, inked typography, brass details, and crayon circuit annotations. One current instruction occupies a repair leaf beside the instrument. Longer optional hints use pages, and the notebook uses three indexed discoveries. See [paper-game-ui.md](paper-game-ui.md) for the generated material and prompt.
