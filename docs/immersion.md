...
The walking HUD has only the chamber marker, reticle and contextual bench interaction. Movement text and notebook/pause buttons are removed, including the notebook button at the circuit bench. N opens the notebook from walking or a circuit; Escape pauses while walking. The title has no reserve-power label or indicator. Closing a modal restores its previous control, with the circuit controls or world canvas as the fallback focus target.
